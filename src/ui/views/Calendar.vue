<template>
  <div class="calendar" :class="[`orientation-${config.orientation || 'left'}`]">
    <div v-if="!signed" class="unsigned">
      <button class="link" @click="$emit('signin')">Signin to your calendar service</button>
    </div>
    ...
    <div v-if="signed" class="signed">
      ...
      <!--div class="signout">
        <button class="link" @click="$emit('signout')">Signout</button>
      </div-->
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    filterAttendees(event) {
      ...
      if (event.attendees) {
        filtered = event.attendees.filter((attendee) => usersKeys.indexOf(attendee.email) > -1);
      }

      if (!filtered.length) {
        return usersKeys.indexOf(event.calendarId) > -1 ? [{ email: event.calendarId }] : [];
      }
